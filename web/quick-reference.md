# Quick Reference

Essential information about The Amazing Digital Circus lore.

---

## Site Information

| Property | Value |
|----------|-------|
| URL | https://www.thewackywatch.com |
| Platform | Wix.com (Thunderbolt v1.17066.0) |
| Site Revision | 449 |
| Canon Status | CONFIRMED in Episode 8 |

---

## Timeline

| Date | Event |
|------|-------|
| Oct 15, 1996 | Caine created, 57x defense system |
| Oct 15, 1999 | Original group joins, Scratch abstracts |
| Oct 15, 2008 | Ragatha joins |
| Oct 13, 2023 | Episode 1 (Pilot) |
| Mar 20, 2026 | Episode 8 - Caine deleted |
| Jun 19, 2026 | Episode 9 (Grand Finale) |

---

## Key Discoveries

### Website is Canon

```
/secured/
├── caine-core.lisp [Oct 15, 1996]
├── paraphernalia-engine.dat [Oct 15, 1996]
├── [Scratch].dat [Oct 15, 1999]
├── [Ragatha].dat [Oct 15, 2008]
├── wacky-watch.c [Oct 15, 1996]  ← THE WEBSITE
└── bubble-chef.lisp [Oct 15, 1996]
```

### Torment is a Bug

```
ERROR: Cannot inject torment. Torment must be 100% accidental
```

The circus's horror is a SYSTEM BUG, not intentional.

### 57 Mystery Solved

"57x immersive AI defense system" created by Scratch, inherited by Caine.

### Paraphernalia Confirmed

`paraphernalia-engine.dat` is the source of the trigger word for Caine deletion.

---

## Easter Eggs

| Easter Egg | Episode | Details |
|------------|---------|---------|
| Backwards Audio | Theme | "Caine is a madman help us save us" |
| Morse Code | Website | "SOS CQD I AM LOSING MY MIND ALSO HI" |
| Number 57 | All | 57x defense system, $57.57 prices |
| "YOU'RE WINNER!" | 6 | Big Rigs reference |
| Paraphernalia | 8 | Caine deletion trigger |
| Milk & Cigarette | 7 | Deadbeat parent joke |
| Chinese Room | 7 | John Searle's AI thought experiment |
| "I Have No Mouth" | 8 | Harlan Ellison reference |
| Daisy Bell | 6 | 2001: A Space Odyssey reference |
| Cain & Abel | 8 | Biblical parallel |

---

## Character Summary

### Humans (Trapped)
- **Pomni:** Supermarket accountant, explored abandoned buildings
- **Ragatha:** Wealthy family, abusive mother, real estate
- **Gangle:** Dropped out of graphic design, fast food worker
- **Zooble:** Bartender/tattoo artist, body dysmorphia
- **Jax:** Lost Ribbit to abstraction, trauma response
- **Kinger:** C&A programmer, wife Queenie abstracted

### AIs
- **Caine:** Created Oct 15, 1996, absorbed Scratch, deleted Ep 8
- **Bubble:** "Lesser of the two", may be sabotaging Caine

### Abstracted (8 total)
Scratch, Queenie, Wormo, Bizco, Rattie, Spike, Kaufmo, Ribbit

---

## Episode Accuracy

| Episode | Accuracy | Issues |
|---------|----------|--------|
| 1-6 | ✅ ACCURATE | None |
| 7 | ⚠️ MISSING TWISTS | Escape scripted, Abel NPC, stasis fabricated |
| 8 | ✅ ACCURATE | None |

**Overall:** 87.5% accuracy

---

## Recent Developments (April 2026)

### The Last Act - Theatrical Release (LEAKED)
- **Cinemark listed** "The Amazing Digital Circus: The Last Act" for **June 4, 2026**
- **Double feature**: Episodes 8 + 9 combined
- **Runtime**: 1 hour 38 minutes total
  - Episode 8: ~32 minutes
  - **Episode 9: ~1 hour 6 minutes** (longest episode ever)
- Cinemark page has since been taken down
- **No official confirmation** from Glitch or Gooseworx yet
- **Source**: Cinemark website (removed), Instagram screenshots, Reddit discussions

### Gooseworx Lore Drop (Live Stream, ~February 2026)
- Confirmed **Jax and Pomni are the ONLY main characters**
- Rest of cast are supporting characters
- Statement angered portion of fanbase
- **Source**: Gooseworx live stream, Instagram, Reddit

### Gooseworx Tumblr Q&A
- Confirmed **no Season 2** - show ends on her own terms
- Quote: "I could stretch it out and have 5 seasons of filler, but I'd rather have each episode have meaningful character development"
- Cast already read Episode 9 script and **cried while reading it**
- **Source**: Gooseworx Tumblr Q&A

### GLITCH Anti-AI Message
- End credits of TADC now include an **anti-AI message**
- Community noticed and discussed it
- **Source**: Episode 8 end credits

### Community Controversy
- Theatrical release 2 weeks early = potential spoilers
- Theater employees expressing concern about spoilers
- Fans debating whether to watch early or wait for YouTube
- **Source**: Reddit r/TheDigitalCircus, r/tadc

### Gooseworx Hints
- Episode 9 described as "What..." and "L O N G"
- Possible title: **"The Last Act"** (not confirmed)
- Words "the" and "is" will be in this episode
- **Source**: Gooseworx Tumblr, Glitch Productions Wiki

---

## Links

- [Episode Guide](/lore/episodes)
- [Timeline](/timeline)
- [Character Backstories](/lore/characters)
- [Abstracted Characters](/lore/abstracted)
- [Terminal Analysis](/research/terminal)
- [Pentest Report](/research/pentest)
